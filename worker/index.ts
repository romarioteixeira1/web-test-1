...
import { appointments, sendDueReminders, type AppointmentBindings } from './appointments'
import type {
  Customer,
  CustomerInput,
  CustomerStats,
...
type Bindings = AppointmentBindings
...
function normalizeTransaction(body: Partial<TransactionInput>) {
  ...
  const installments =
    payment_status === 'parcelado' && Number.isFinite(body.installments) && Number(body.installments) > 0
